...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.image_paths = []
        self.metric = 0
        self.curr_epoch = 0 # current epoch count
        self.curr_iter = 0 # current iteration count
        if self.isTrain:
            self.ignore_label = opt.ignore_label
        # get palette for grayscale colorization
        palette = {}
        with open(opt.palette, 'r') as f:
    ...
    def get_current_visuals(self):
        ...
        for name in self.visual_names:
            if isinstance(name, str):
                if 'color' in name:
                    visual_ret[name] = self.colorize_single_img((getattr(self, name)))
                else:
    ...
    def colorize_single_img(self, img):
        img = img.data
